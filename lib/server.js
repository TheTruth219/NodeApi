...
let urlTools = require('./config').templateGlobals;
...
server.unifiedServer = (req,res) => {
  ...
  urlTools.setBaseUrl(hostname);
  let pathname = url.parse(req.url).pathname;
  console.log(`${urlTools.getBaseUrl()}${pathname}`);
...
// Init script
server.init = ()=>{
  let finalPort = process.env.PORT || config.httpPort;
  let finalSecuredPort = process.env.PORT || config.httpsPort
  // Start the server, and have it listen on port 3000
  server.httpServer.listen(finalPort, ()=>{
    console.log('\x1b[36m%s\x1b[0m',`The server is listening on port ${finalPort}`)
  });
